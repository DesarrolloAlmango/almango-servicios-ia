...
const CategoryCarousel: React.FC<CategoryCarouselProps> = ({
  ...
  useEffect(() => {
    ...
    itemRefs.current.forEach((ref, id) => {
      if (categories.find(c => c.id === id)) { // Only observe existing categories
  ...
  const saveImageToCache = useMemo(() => {
    ...
    return (categoryId: string, imageData: string) => {
      ...
      debounceTimer = setTimeout(() => {
        try {
          const currentCache = { ...cachedImages, [cacheKey]: imageData };
            ...
            return (
              ...
              >
                <div 
                  ...
                  className={`cursor-pointer hover:scale-105 transition-transform mx-5px ${isSelectedCategory(category.id) ? 'ring-2 ring-primary ring-offset-2' : ''}`} 
                ...
                >
                  <div className="overflow-hidden rounded-full border-2 border-primary mx-auto w-16 sm:w-20 h-16 sm:h-20 mb-2 bg-gray-100 relative">
                    <AspectRatio ratio={1} className="bg-gray-100">
                      ...
                      {cachedImages[cacheKey] && !failedImages[category.id] ? (
                        <img 
                          src={cachedImages[cacheKey]} 
                          alt={category.name} 
                        ...
                        <>
                          ...
                          {getImageSource(category.image, category.id) && !failedImages[category.id] ? (
                            <img 
                              ...
                              data-src={getImageSource(category.image, category.id)} 
                              alt={category.name} 
                              className="w-full h-full object-cover" 
                              loading="lazy" 
                  ...
                  <p className="text-center text-sm font-medium mt-1 sm:mt-2 line-clamp-2 animate-in fade-in duration-300 px-0 sm:text-sm">
                    {category.name}
                  </p>
                </div>
              </CarouselItem>
