...
const ProductGrid: React.FC<ProductGridProps> = ({
  ...
  const checkProductPermission = async (commerceId: string, serviceId: string, categoryId: string, productId: string): Promise<boolean> => {
    try {
      ...
      const url = `${baseUrl}/WebAPI/ORubroItemActivo?Comercioid=${commerceId}&Nivel0=${serviceId}&Nivel1=${categoryId}&Nivel2=${productId}&Nivel3=0`;
    ...
    try {
      ...
      const initialProducts = productsData.map((product: any) => {
        ...
        return {
          ...
          price: 0,
          // Initialize with 0 to show loading state
